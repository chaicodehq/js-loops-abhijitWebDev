...
export function diwaliLightsPlan(lightStrings, budget) {
  ...
  // add all items with calculated cost
  for(let item of lightStrings) {
    let rate;

    // condition, if color of item is golden then rate is 50
    if(item.color === 'golden') rate=50;

     // if it is multicolor rate will be 40
   else if(item.color === 'multicolor' ) rate = 40

   // color is white, then rate is 30
  ...
  // Step 2: while loop - remove from END until within budget
  while(totalCost > budget && selected.length) {
    const removed = selected.pop();
    totalCost -= removed.cost;
  }

  // calculate totalLength of whatever is remaining
let totalLength = 0;
